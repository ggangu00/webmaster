...
public class BookMain {
	...
	static void memberManage() { //회원관리
		...
		while(run1) {
			System.out.println("--------------------------------------------------------------------------");
			System.out.println("1.회원등록 | 2.회원검색 | 3.회원전체 조회 | 4.회원삭제 | 5.회원 정보 변경 | 6.프로그램 종료");
			System.out.println("--------------------------------------------------------------------------");
			System.out.print("▶ 번호 선택 : ");
			int temp = sc.nextInt();
			sc.nextLine();
			...
			switch(temp) {
				case 1:				
					...
					System.out.print("권한> ");
					String responsibiliy = sc.nextLine();
					
					Member member = new Member(memberId, password, member_name, phone, responsibiliy);
				...
				case 2:		
					...
					List<Book> list = mdao.memberList(search);
					for(Book bk : list) {
						System.out.println(bk.toString());
					}
					break;
